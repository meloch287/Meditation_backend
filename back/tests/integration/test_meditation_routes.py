...
def test_seed_med(client: TestClient, db_session: Session):
    meditations = [Meditation(title=f"Med {i}", description="Desc", duration_seconds=300, audio_url="url", is_premium=False, category="Сон") for i in range(3)]
    ...
    data = resp.json()
    assert all(m["is_premium"] == False for m in data)
...
def test_premium_access(client: TestClient, db_session: Session):
    ...
    db_session.add(user)
    meditations = [Meditation(title=f"Med {i}", description="Desc", duration_seconds=300, audio_url="url", is_premium=(i%2==0), category="Сон") for i in range(6)]
    db_session.add_all(meditations)
    db_session.commit()
...
def test_filtered_premium(client: TestClient, db_session: Session):
    ...
    db_session.add(user)
    med1 = Meditation(title="Free", description="Free", duration_seconds=300, audio_url="url1", is_premium=False, category="Сон")
    med2 = Meditation(title="Premium", description="Premium", duration_seconds=600, audio_url="url2", is_premium=True, category="Сон")
...
def test_last_played(client: TestClient, db_session: Session):
    med = Meditation(title="Meditation 1", description="Desc", duration_seconds=300, audio_url="url", is_premium=False, category="Сон")
    db_session.add(med)
    db_session.commit() 
